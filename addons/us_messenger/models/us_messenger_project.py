...
class UsMessengerProject(models.Model):
    ...
    is_hide_change_operator_user = fields.Boolean('Hide button "Change Operator" for operators',
    ...
    def extract_fields(self, root, model_name, record):
        ...
        for field in data:
            ...
            if field_name == "common_developer_code" and model_name == "us.messenger.project":
                ...
                field_value = self.check_bool(field.get("eval")) if model_name not in [
                    "us.messenger.trigger.automation", "us.messenger.trigger.webhook"] else True
    ...
    def write(self, vals):
        ...
        if 'operator_ids' in vals:
            ...
            bot_channels = self.env['discuss.channel'].search(
                [('id', 'in', channels_ids), ('messenger_operator_id.id', 'in', partners)])
